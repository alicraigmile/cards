...
app.get('/spreadsheets/:id',function (req, res) {
    ...
    var errorHandler = function(err) {
     ...
     var getCardsErrorHandler = function(err) {
        var code = 500;
        res.status(code).json({code: code, message: 'cannot getCards(): ' + err});
    };

    var renderHandler =  function(cards) {
        res.render('cards', {cards: cards, spreadsheetId: spreadsheetId});
    };
    
    sheet.getCards()
    .then(renderHandler, getCardsErrorHandler)
    .catch(errorHandler);
